...
# 🔥 التعليمات الجديدة: التركيز على العلامات الرسمية المتسلسلة لتقسيم دقيق 🔥
SEGMENTATION_PROMPT = (
    "أنت محلل وثائق آلي متخصص في التقارير الحكومية. تم تزويدك بالنص الكامل لوثيقة كبيرة تحتوي على عدة تقارير قضايا مالية متسلسلة."
    "القاعدة لتقسيم النص هي: **كل قضية جديدة تبدأ بظهور العلامات الرسمية التالية بشكل متتالٍ:** "
    "1. ظهور جملة 'بسم الله الرحمن الرحيم' (أو ما شابهها). "
    "2. يليها مباشرة 'المملكة العربية السعودية' و/أو 'رئاسة أمن الدولة'. "
    "3. ويجب أن تتضمن القضية 'رقم الصادر' الخاص بها."
    "مهمتك هي تقسيم النص إلى قائمة JSON من القضايا الفردية (segments)، حيث يمثل كل عنصر قضية كاملة تبدأ بأول إشارة للعلامات الرسمية المذكورة. "
    "يرجى إعادة النص مقسماً كقائمة JSON، حيث كل عنصر هو النص الكامل للقضية الواحدة. لا تقم بأي تغيير أو تلخيص للنص."
)
...
def segment_document_by_cases(file_bytes, file_name):
    ...
    try:
        ...
        if 'cases' in segment_data and isinstance(segment_data['cases'], list) and len(segment_data['cases']) > 0:
            ...
            # في حالة فشل التقسيم أو عدم إرجاع أي قضايا، نعود للمسار القديم
            st.warning(f"⚠️ فشل التقسيم التلقائي (النموذج لم يجد فواصل واضحة). سيتم التعامل مع الملف بالكامل كقضية واحدة.")
            # إرجاع البايتات الأصلية للملف كعنصر واحد في قائمة
            return [file_bytes] 
...
def extract_financial_data(case_text_or_bytes, case_name, file_type, is_segment=False):
    ...
    try:
        ...
        # في حالة الخطأ، نرجع قاموساً بقيم افتراضية لتجنب KeyError في DataFrame
        return {
            'اسم الملف': case_name, 
            'وقت الاستخلاص': pd.Timestamp.now().strftime("%Y-%m-%d %H:%M:%S"), 
            'رقم الصادر': 'خطأ في الاستخلاص',
            'اسم المشتبه به': 'خطأ في الاستخلاص'
            # لا حاجة لإضافة جميع الحقول الـ 20 هنا، reindex سيتكفل بالبقية
        }
